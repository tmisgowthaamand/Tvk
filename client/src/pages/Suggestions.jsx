...
export default function Suggestions() {
    ...
    const load = useCallback(async () => {
        setLoading(true)
        try {
            const res = await apiGet('/suggestions', { page, limit: 20, search, status })
    ...
    const handleUpdate = async (id, newStatus, notes) => {
        await apiPatch(`/suggestions/${id}`, { status: newStatus, notes })
        setModal(null)
        load()
    }
    ...
    return (
        <div className="page fade-in">
            <div className="page-header">
                <h1>💡 Suggestions</h1>
                <p>Voter suggestions received via WhatsApp</p>
            </div>
                                    ...
                                    <tr key={s._id}>
                                        ...
                                        <td>
                                            <div className="action-btns">
                                                <button className="btn btn-sm btn-primary" onClick={() => setModal(s)}>Update</button>
                                                <button className="btn btn-sm btn-wa" onClick={() => { setNotifyModal(s); setNotifyMsg(`Hi ${s.voterName}, regarding your suggestion (${s.suggestionId}):\n\n`) }}>
            ...
            {modal && <UpdateModal item={modal} onUpdate={handleUpdate} onClose={() => setModal(null)} statuses={['Pending', 'Reviewed', 'Implemented']} />}
